...
function clearForm(){
           ...
           // $('#expired_on').val('01/01/2020');
			...
			$("#interval").val('0');
			$('#profile_image').attr('src','instagram_profile_avatar.jpg')
...
function populateTable () {
            $.get('http://192.168.18.20:8000/core/get_facebook_targets', function (data) {
            ...
            $('#smart_serach').on('click', function () {
                ...
                if (author_account_name == '') {
                    ...
                    $.ajax({
                        type: 'GET',
                        dataType: 'json',
                        url: 'http://192.168.18.20:8000/core/smart_search/' +author_account_name + '/facebook/',
...
//end smart search 
            ...
            $('#submitBTN').on('click', function (e) {
                ...
                if (valid == false ) {
                 ...
                 //   let is_expired = $('#is_expired').is(':checked')
                    ...
                    fetch('http://192.168.18.20:8000/core/add_facebook_target/', {
                            method: "POST",
                            cache: "no-cache",
                            headers: {
						...
						populateTable();
                            $('#smart_search_err').show();
                           $('#smart_search_err').empty().append('New Facebook Author Target  Successfull Added ');
                           $('#smart_search_err').fadeOut(3000);
							}
							else{
								$('#smart_search_err').show();
    ...
    <script>
        $(document).ready(function () {
            ...
            $.get('http://192.168.18.20:8000/core/add_facebook_target/', function (data) {
